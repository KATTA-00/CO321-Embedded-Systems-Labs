...
#define DCT_CI_MACRO(n, A) __builtin_custom_ini(ALT_CI_DCT_COMPONENTB_N + (n & 0b11111), (A))
...
void DCT(INT16 *data)
{
	...
	{
		temp = data[i];
		WRITE_FIFO(&temp, IN_BASE_3to4B, CONTROL_BASE_3to4B); // Casting data
	}
}
...
int main(void)
{
	...
	while (1)
	{
		...
		printf("DCT Started\n");
		READ_FIFO(&size, OUT_BASE_1to2B, CONTROL_BASE_1to2B);
